...
const viewDepartment = () => {
    console.log('Viewing by Deparment');
...
const viewAllRoles = () => {
    ...
    connection.query(`SELECT roles.title, roles.id, department.name AS department, roles.salary, CONCAT(employee.first_name, " ", employee.last_name) AS employee
                     FROM roles
                     LEFT JOIN department ON department.id = roles.department_id
                     RIGHT JOIN employee ON roles.id = employee.role_id;`, function(err, res) {
        if (err) throw err;
        console.table(res);
    });
    presentOptions;
...
const viewAllEmployees = () => {
    ...
    connection.query(`SELECT employee.id, CONCAT(employee.first_name, " ", employee.last_name) AS employee, roles.title, department.name AS department, roles.salary, CONCAT(manager.first_name, ' ', manager.last_name) AS manager 
                    FROM employee
                    LEFT JOIN employee manager ON manager.id = employee.manager_id
                    LEFT JOIN roles ON roles.id = employee.role_id
                    LEFT JOIN department ON department.id = roles.department_id;`, function(err, res) {
        if (err) throw err;
        console.table(res);
    });
    presentOptions();
